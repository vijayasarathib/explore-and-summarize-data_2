...
```
quality level of 6 to 7 has a highest number of wine. we can also observe that there is no alchol whose quality level is less than 3 and greater than  9.
...
```
pH value varies like a bell curve, though it has a very less value in the begining we do see that the peak value reaches around 3 and quantity decreases for the higher pH values.
...
```
correlation of -.3 didnt give any clear picture so we went to create a plot to understand the data.
...
```
By looking at the scatter plot, i dont we get much of the insight about the chlorides, however over all from the cartesian axis it looks clear that the chlorides increses over slight increase of the density. or the other way density related with the chlorides is obvious.
...
```
minimum level of pH is 2.9. may be if we take the bigger data set we can realise that the high quality wines having higher alcohol and ph value. but in our sample data set we have more of the quality level 6 wines and of these ph is high and alcohol too.
...
it shows off clearly that the people like the wine better if we have higher alcohol level. however other parameters also giving closer impact.
